...
class PaymentController
{
    ...
    public function testPayment(int $reservationId)
    {
        ...
        $item = new \Laravel\Cashier\Charge\ChargeItemBuilder($reservation);
        $item->unitPrice(money(20000, 'EUR')); //1 EUR
        $item->description('energie-10-06-2022');
        $chargeItem = $item->make();

        $item2 = new \Laravel\Cashier\Charge\ChargeItemBuilder($reservation);
        $item2->unitPrice(money(0, 'EUR'));
        $item2->description('Test Item 4');
        $chargeItem2 = $item2->make();

        $result = $reservation->newCharge()
            ->addItem($chargeItem)
            ->addItem($chargeItem2)
            ->setRedirectUrl('/mollie.nl')
            ->create();
